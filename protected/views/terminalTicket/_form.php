...
function addSelect(){
    ...
    var newSelect= $('<div id=container'+current+'>'+current+
     ...
     'FirstName <input class="span2" id=fname_'+current+' type=text name="Passenger[first_name][]">'+
     'LastName<input class=span2 id=lname_'+current+' type=text  name="Passenger[last_name][]">'+
     'Age <input class=span1 id=age_'+current+' type=text  name="Passenger[age][]">'+
     '<br></div>');
...
<?php if($bn):?>
   var url = '<?=Yii::app()->createUrl('terminalTicket/tkt_print',array('type'=>'tkt','bn'=>$bn))?>';
   window.open(url);
 <?php endif;?>
